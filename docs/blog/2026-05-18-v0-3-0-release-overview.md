...
**TL;DR**: The new [/prompts/](/prompts/) page composes benchmarking instructions for coding agents. `joinorder` now uses the real IMDb 2013 Join Order Benchmark dataset at scale factor 1, replacing synthetic data. BenchBox also expands approximate-aggregate and sketch-lifecycle coverage in the primitives benchmarks.
...
This release adds a [new prompt composer page](/prompts/) where you can "Instruct a coding agent to use BenchBox."
