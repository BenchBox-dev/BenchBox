...
DuckDB is capped below the next major (`<2.0.0`) across the development, `all`,
`duckdb`, and `explorer` dependency paths, so a future 2.x API break surfaces in
CI rather than silently during `uv sync`. Browser readability is **not** a
constraint on the minor version: DuckDB v1.x writers default to the v1.0.0
on-disk storage format (version 64), which `@duckdb/duckdb-wasm` attaches
regardless of the writer's version. A newer storage format is only emitted if a
writer explicitly opts into `storage_compatibility_version`, which the explorer
snapshot builder does not.
