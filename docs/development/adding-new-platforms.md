...
coordinates. The module uses only the Python standard library; reading catalog
metadata must not import an optional SDK or instantiate an adapter.

The remaining platform-shaped maps have narrower runtime ownership:

- `PlatformRegistry` derives metadata, only registry-scoped compatibility
  aliases, and lazy adapter imports from the manifest, then owns loaded adapter
  classes and availability state. Its public adapter list preserves the
  manifest's explicit `registration_order`.
- CLI platform normalization derives only CLI-scoped aliases. A `-df` spelling
  must also declare `implied_mode: dataframe`; it is not automatically a core
  registry alias.
- platform hooks, local DataFrame factories, tuning aliases, and file-format
  capabilities describe behavior rather than platform identity. They remain
  local to their subsystem, but `make platform-manifest-check` rejects keys or
  aliases that do not resolve through the manifest.
- `benchbox.platforms` lazy exports are API exposure. They are checked against
  manifest runtime classes or the DataFrame factory, not treated as another
  registry.
- Every DataFrame factory row is checked for a CLI-scoped, DataFrame-implying
  spelling, DataFrame-capable canonical target, real lazy adapter export,
  real lazy availability constant, and nonempty installation guidance.

Do not add a second canonical-key, alias, capability, support-status, or
...
<!-- BEGIN GENERATED PLATFORM MANIFEST -->
...
|---|---|---|---|---|
| `athena` | — | SQL | `beta` | #32 `benchbox.platforms.athena:AthenaAdapter` |
...
| `clickhouse-server` | — | SQL | `beta` | #8 `benchbox.platforms.clickhouse_server:ClickHouseServerAdapter` |
| `cudf` | `cudf-df` (cli, mode=dataframe) | DataFrame | `experimental` | DataFrame factory only |
| `dask` | `dask-df` (cli, mode=dataframe) | DataFrame | `beta` | DataFrame factory only |
| `databend` | — | SQL | `beta` | #27 `benchbox.platforms.databend:DatabendAdapter` |
| `databricks` | `dbx` (cli) | SQL + DataFrame | `beta` | #4 `benchbox.platforms.databricks:DatabricksAdapter` |
| `databricks-df` | — | SQL + DataFrame | `experimental` | #5 `benchbox.platforms.databricks:DatabricksDataFrameAdapter` |
| `datafusion` | `datafusion-df` (cli, mode=dataframe), `fusion` (cli) | SQL + DataFrame | `stable` | #3 `benchbox.platforms.datafusion:DataFusionAdapter` |
| `dataproc` | — | SQL + DataFrame | `experimental` | #36 `benchbox.platforms.gcp:DataprocAdapter` |
| `dataproc-serverless` | — | SQL + DataFrame | `experimental` | #37 `benchbox.platforms.gcp:DataprocServerlessAdapter` |
| `doris` | — | SQL | `beta` | #28 `benchbox.platforms.doris:DorisAdapter` |
| `duckdb` | `duck` (cli) | SQL | `stable` | #0 `benchbox.platforms.duckdb:DuckDBAdapter` |
| `ducklake` | — | SQL | `beta` | #2 `benchbox.platforms.ducklake:DuckLakeAdapter` |
| `emr-serverless` | — | SQL + DataFrame | `experimental` | #34 `benchbox.platforms.aws:EMRServerlessAdapter` |
| `fabric-lakehouse` | `fabric_lakehouse` (registry) | SQL | `beta` | #39 `benchbox.platforms.fabric_lakehouse:FabricLakehouseAdapter` |
| `fabric-spark` | — | SQL + DataFrame | `experimental` | #38 `benchbox.platforms.azure:FabricSparkAdapter` |
| `fabric_dw` | `fabric-dw` (cli/registry) | SQL | `beta` | #31 `benchbox.platforms.fabric_warehouse:FabricWarehouseAdapter` |
| `firebolt` | — | SQL | `beta` | #26 `benchbox.platforms.firebolt:FireboltAdapter` |
...
| `presto` | `prestodb` (cli) | SQL | `beta` | #17 `benchbox.platforms.presto:PrestoAdapter` |
| `pyspark` | `pyspark-df` (cli, mode=dataframe) | SQL + DataFrame | `beta` | #25 `benchbox.platforms.pyspark:PySparkSQLAdapter` |
| `quanton` | — | SQL + DataFrame | `experimental` | #46 `benchbox.platforms.onehouse:QuantonAdapter` |
| `questdb` | — | SQL | `beta` | #22 `benchbox.platforms.questdb:QuestDBAdapter` |
| `redshift` | `rs` (cli) | SQL | `beta` | #13 `benchbox.platforms.redshift:RedshiftAdapter` |
| `singlestore` | — | SQL | `beta` | #29 `benchbox.platforms.singlestore:SingleStoreAdapter` |
| `snowflake` | `snow` (cli) | SQL | `beta` | #14 `benchbox.platforms.snowflake:SnowflakeAdapter` |
| `snowpark-connect` | — | SQL + DataFrame | `experimental` | #45 `benchbox.platforms.snowpark_connect:SnowparkConnectAdapter` |
| `spark` | — | SQL | `beta` | #41 `benchbox.platforms.spark:SparkAdapter` |
| `sqlite` | `sqlite3` (registry) | SQL | `stable` | #11 `benchbox.platforms.sqlite:SQLiteAdapter` |
| `starburst` | — | SQL | `beta` | #16 `benchbox.platforms.starburst:StarburstAdapter` |
| `starrocks` | — | SQL | `beta` | #10 `benchbox.platforms.starrocks:StarRocksAdapter` |
| `synapse` | `azure-synapse` (cli), `azure_synapse` (registry), `azuresynapse` (cli) | SQL | `beta` | #24 `benchbox.platforms.azure_synapse:AzureSynapseAdapter` |
| `synapse-spark` | — | SQL + DataFrame | `experimental` | #40 `benchbox.platforms.azure:SynapseSparkAdapter` |
| `timescaledb` | — | SQL | `beta` | #19 `benchbox.platforms.timescaledb:TimescaleDBAdapter` |
| `trino` | `trinodb` (cli) | SQL | `beta` | #15 `benchbox.platforms.trino:TrinoAdapter` |
| `velox` | — | SQL | `experimental` | #43 `benchbox.platforms.velox:VeloxAdapter` |
<!-- END GENERATED PLATFORM MANIFEST -->
...
Do not add parallel entries to `PlatformRegistry` or the CLI. They derive their
metadata, scoped aliases, and runtime import rows from the manifest. Use
`registry` scope only for core API compatibility spellings; use `cli` for CLI
shorthands. If an alias implies DataFrame execution, declare
`"implied_mode": "dataframe"`. Preserve existing registration order and assign
the next contiguous integer to a new runtime adapter. `support_status` is a
product support classification, not local dependency availability.
...
If the platform has subsystem behavior, add only the relevant local entries:
platform option specs/config builders, DataFrame factory routing, tuning
