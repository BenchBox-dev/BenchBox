...
| `make/worktrees.mk` | Pool initialization, claim, and release wrappers | Keep the pre-`worktree-release-locked` portion of the lifecycle here; the literal-parser-pinned locked release recipe remains in root |
| `make/worktree-pool.mk` | Pool status, consistency checking, and reset wrapper | Keep this module between the locked release and locked reset recipes so variable and rule evaluation retains the monolith's parse order |
| `make/worktree-maintenance.mk` | Pool sweep, disk cleanup, list/prune, finding, and soundness utilities | Keep post-reset maintenance and reporting here; the literal-parser-pinned locked reset recipe remains in root |
