...
| `_project/make/platform-tests.mk` | Credentialed live-platform and local compose/container test lifecycle | Add platform-specific test entry points and their directly coupled engine variables/macros here |
| `_project/make/documentation.mk` | Docs build/validation, generated compatibility docs, platform manifest, parity fixtures, and native TPC binary compilation | Add documentation or generated-contract targets here; do not place general lint targets here |
| `_project/make/worktrees.mk` | Pool initialization, claim, and release wrappers | Keep the pre-`worktree-release-locked` portion of the lifecycle here; the literal-parser-pinned locked release recipe remains in root |
| `_project/make/worktree-pool.mk` | Pool status, consistency checking, and reset wrapper | Keep this module between the locked release and locked reset recipes so variable and rule evaluation retains the monolith's parse order |
| `_project/make/worktree-maintenance.mk` | Pool sweep, disk cleanup, list/prune, finding, and soundness utilities | Keep post-reset maintenance and reporting here; the literal-parser-pinned locked reset recipe remains in root |
| `_project/make/help.mk` | Exact ordered `make help` recipe | Preserve existing category and command order; add a help line in the same change as a new user-facing target |
| `_project/make/inventory.json` | Current evaluated target, alias, prerequisite, recipe, variable, macro, statement-order, default-goal, and include-order contract | Regenerate only for an intentional permitted contract change and review the manifest diff |
| `_project/make/monolith-baseline.json` | Durable inventory of the committed pre-split monolith: 198 targets, 195 public targets, and default goal `test` | Do not regenerate during ordinary Make changes; changing this migration proof requires a separate architectural decision |
| `_project/make/migration-proof.json` | Compact hashes, counts, include order, and reviewed delta for the initial split | The normal inventory writer never changes this file; preserve it with the monolith baseline as historical evidence |
...
| `tests/unit/test_release_infrastructure.py`, `tests/unit/workflows/test_auto_merge_enablement_point.py`, and `tests/unit/test_auto_merge_soundness_paths.py` | Parse release, PR, auto-merge, worktree-release, package, and UAT gate source text | Those operational safety recipes remain root-owned |
...
The separate migration proof records that the initial extraction added only the
six ordered includes, the non-overridable `BENCHBOX_MAKEFILE_ROOT` bootstrap,
the public phony `makefile-inventory-check` target, and that target's single help
line. Its normalized semantic hash equals the monolith's hash; the reviewed
delta changes the baseline counts from 198/195 to 199/196.
...
```bash
uv run -- python _project/scripts/check_makefile_inventory.py --write
git diff -- Makefile _project/make/
make makefile-inventory-check
```
...
```bash
uv run -- python _project/scripts/check_makefile_inventory.py --verify-migration
```
