...
| `benchbox/core/runner/dataframe_runner.py` | Removed. `dataframe_compliance_class` and `no_dataframe_queries_message` moved to `benchbox/platforms/dataframe/benchmark_mixin.py` (their only production consumer); the standalone legacy runner, its parameter-override machinery, and its compatibility tests are deleted. Production DataFrame execution uses `adapter.run_benchmark()` plus `BenchmarkExecutionMixin` | removed | Completed by delete-benchbox-core-runner-dataframe-runner-py-completely: helper imports migrated, legacy tests retired or re-pointed at the production path | The adapter-mixin path is the only production DataFrame lifecycle promise | dataframe-runtime |
