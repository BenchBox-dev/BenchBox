...
**Corpus:** the browser fixture corpus is generated by `npm run test:e2e:fixtures` from four source bundles in `results-explorer/test-fixtures/source/bundles/`: `tpch` on `duckdb`, `datafusion`, and `polars`, plus `star_schema` on `duckdb`, all at SF 0.01. The generator adds derived variants (the only second-scale variant is a `tpch-duckdb` SF 0.1 record, plus community-submission, tuned, partial-coverage, and synthetic cloud-environment records). `star_schema` has no SF 0.1 variant. There is **no** `sqlite` and no `star_schema` on non-`duckdb` platforms — expect those filter combinations to produce empty cohorts, not errors. Exact result IDs change whenever the fixtures regenerate; take current IDs from the Browse view rather than assuming any listed here.
...
E2E_PAGES_SHAPED=1 \
E2E_PORT=<allocated-port> npx playwright test --project=chromium --workers=1 e2e/routes/pages-artifact.spec.ts
```
...
### S2.1 Stat cards
- Three cards: Results / Benchmarks / Platforms. Numbers match the default fixture corpus (12 results, 2 benchmarks, 8 platforms — confirmed by running `npm run test:e2e:fixtures` and querying the generated `results.duckdb`; re-verify if the generator's variant list changes).
...
### S3.2 Scale filter
- Scale options come from the current benchmark cohort: `tpch` has 0.01 and 0.1; `star_schema` has 0.01 only. Switching an available value changes URL `?sf=…` and re-fetches the cohort.
...
Candidate picking no longer renders at `/results/compare`. The builder shows the pinned result (when `?ids=<one>`) and a link to Query. Selecting the second run happens in Query's paged, searchable table; `/results/compare?ids=<a,b>` remains the comparison renderer only. Document height assertions use the large fixture (rx-13), not the default 12-result fixture.
