...
| Define benchmark family plugin seam before core package extraction | Pilot complete on SSB; further migration stopped pending new evidence | benchmark-api | Keep public wrapper tests and the SSB plugin contract green; do not migrate another family until a runtime consumer of `phases()` / `result_metadata()` or a measured family-migration cost table provides new evidence. | `docs/design/future-state/benchmark-family-plugin-seam/README.md`, `docs/reference/public-contracts.md`, `benchbox/core/benchmark_loader.py`, `benchbox/core/ssb/family.py` |
