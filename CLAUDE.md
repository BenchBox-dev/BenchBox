...
- **TODO** (read, auto-allowed): `_project/scripts/todo list*|show*|stats|ready|deps*|lint*|export*|check-scope*`, `uv run --project _project/scripts -- python _project/scripts/todo_db.py list*|show*|stats|ready|deps*|lint*|export*|check-scope*`. Write/lifecycle subcommands (`claim`/`start`/`done`/`defer`/`complete`/…) and `verify --run` run in feature/pool worktrees per the PR/Worktree write policy.
...
- **UAT**: `make uat-cell PLATFORM=* BENCHMARK=* SCALE=*`, `make uat-stress`, `make uat-sweep CONFIG=*`, `make uat-execute CONFIG=*`, `make uat-validate RESULTS_DIR=* OUTPUT_TSV=*`, `make uat-package CONFIG=* SUBMISSIONS_DIR=* RESULTS=*`, `make uat-explorer-smoke BUNDLES_DIR=* OUTPUT_DIR=* LOG_DIR=*`, `make uat-report CELLS_JSONL=* OUTPUT_TSV=*`, `make uat-bring-up PLATFORM=*`, `make uat-docker-cleanup`
