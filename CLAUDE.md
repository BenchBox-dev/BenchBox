...
- **UAT framework — local-only / read-only** (operator targets; see `docs/operations/uat-framework.md`): `make uat-validate RESULTS_DIR=* OUTPUT_TSV=*`, `make uat-report CELLS_JSONL=* OUTPUT_TSV=*`, `make uat-explorer-smoke BUNDLES_DIR=* OUTPUT_DIR=* LOG_DIR=*`. These read existing artefacts or build the explorer locally; no external service or billable platform reachable.
- **UAT framework — potentially-billable, prompt-on-use** (NOT auto-allowed): `make uat-cell PLATFORM=*`, `make uat-stress`, `make uat-sweep CONFIG=*`, `make uat-execute CONFIG=*`, `make uat-package CONFIG=*`. These call `benchbox run` or `benchbox submit`, which can resolve `--platform` to billable cloud targets (snowflake, databricks, motherduck, clickhouse-cloud, athena, redshift, bigquery). Always confirm with the user before invoking — even when the YAML config or stress preset *appears* local-only, an `apply_stress_overrides` PLATFORM= override or a frozen replay can route to a remote platform.
