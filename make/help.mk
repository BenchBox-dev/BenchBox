...
help:
	...
	@echo "  make coverage-all    Run routine coverage (excludes stress/resource-heavy/live tests)"
	@echo "  make coverage-opt-in-all Run full tree incl. stress/resource-heavy/live (needs services+creds)"
	...
	@echo "  make pr-open [PR_BODY_FILE=path] [READY=1] [STALE=1]  Push branch + open PR vs develop (READY=1 arms only a reused reviewed PR; new PRs stay held)"
	...
	@echo "  make pr-refresh      Merge origin/develop into current branch and re-run pr-open (READY=1 arms only a reused reviewed PR)"
	...
	@echo "  make worktree-finish WORKTREE_PATH=path EXPECTED_HEAD_OID=oid [FORMAT=json] [APPLY=1]  Preview single-target removal and ref deletion (APPLY=1 executes)"
