...
Suppose one benchmark result reports 12 seconds and another reports 31 seconds. Before treating that as a performance comparison, we need more information. Did both runs use the same benchmark and scale factor? Did they execute the same phase and enough of the same queries? Did any query fail? Were the runs SQL or DataFrame executions? What hardware, software version, tuning, and validation evidence did each record? Who ran them, and who funded the work?

The [Results Explorer](https://benchbox.dev/results/) launched as a curated preview with BenchBox v0.4.0.[^release] Its main engineering job is not sorting numbers. It is deciding which claims the available evidence supports, then showing the evidence gaps that remain.

## The launch snapshot has visible limits
...
The validation breakdown was 56 passed, 30 partial, and 52 not run. Only 11 results recorded a complete environment across operating system, architecture, CPU count, memory, and Python version. None recorded a driver version, and only eight recorded a tuning mode. Every result was labeled `maintainer-run`, with funding `unspecified`.
...
That is why the preview needs more than a leaderboard. A polished ranking without those limitations would make the corpus look more complete than it is.
...
The code also names `trend_safe` and `provenance_only` chart classes. They are labels for datasets, not additional filters: both return all rows. Calling all six classes gates would overstate what the implementation enforces.

## Hard gates and visible warnings are not the same
...
| Tuning and cost metadata | Recorded differences appear as warnings |
...
The Comparability Receipt then checks workload, date, platform version, driver version, execution mode, validation, environment, tuning, and cost. A difference gets a warning. A field that nobody recorded gets a neutral missing status. This distinction matters: a platform-version warning may coexist with a displayed winner claim. The receipt does not certify that the comparison is methodologically equivalent. It gives the reader the recorded differences needed to decide whether the claim is useful.

That design is intentionally narrower than saying "comparable" means "same environment." The launch corpus does not have enough complete environment records to enforce that rule without removing most of the site. Instead, the Explorer blocks the mismatches it can define reliably and exposes the rest.
...
## The corpus needed two corrections
...
The cost was visible. AMPLab, CoffeeShop, H2O-DB, SSB at SF1, and TSBS DevOps groups left the published corpus. A group can return when it has at least three truthful platform results. Fresh DataFrame results also need real validation evidence.
...
On August 28, it returned 52 not-run and non-ranking results, 30 partial and non-ranking results, one passed but non-ranking result, and 55 passed and ranking-eligible results.
...
Result submissions are not the call to action for this preview. The public contribution instructions still describe that path as closed. Until it opens, the useful actions are to browse, audit, and [open an issue](https://github.com/BenchBox-dev/BenchBox/issues) when a result or explanation needs correction.
...
[^eligibility]: [Display and comparison eligibility implementation at v0.4.0](https://github.com/BenchBox-dev/BenchBox/blob/v0.4.0/results-explorer/src/lib/displayEligibility.ts).
[^compare]: [Compare page cohort and claim-suppression logic at v0.4.0](https://github.com/BenchBox-dev/BenchBox/blob/v0.4.0/results-explorer/src/pages/Compare.tsx) and [Comparability Receipt implementation](https://github.com/BenchBox-dev/BenchBox/blob/v0.4.0/results-explorer/src/components/ComparabilityReceipt.tsx).
[^database]: [Results Explorer database attachment at v0.4.0](https://github.com/BenchBox-dev/BenchBox/blob/v0.4.0/results-explorer/src/db.ts).
[^corpus]: [Corpus generation and correction notes at v0.4.0](https://github.com/BenchBox-dev/BenchBox/blob/v0.4.0/results-data/CORPUS_NOTES.md).
