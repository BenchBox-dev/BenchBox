...
**Summary:** 10 dual-surface candidates (unguarded + staged, registered but not CI-enforced) — 0 cross-surface gateable (verified verbatim id overlap at a bounded scale), 3 candidate-unverified (registry exists but ZERO verified id overlap — needs a confirmed id mapping first), 2 not-cheaply-gateable (rejects a bounded scale or needs a canonical fetch), 4 have no DataFrame query surface (need a w2 fallback oracle), 1 abandoned (correspondence investigated and ruled out), 0 blocked.
