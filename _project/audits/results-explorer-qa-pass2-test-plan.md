...
**Server:** `http://localhost:5173/results/` (Vite dev, already running in the background — no need to start it; if it ever drops, run `cd results-explorer && npm run dev` from the canonical clone)
**Branch under test:** `chore/todo-review-nits-dry-run-2026-04-29`
...
This pass supersedes pass-1. The goal is full functional + usability coverage of every control and configuration on every page, not just the four bugs we already know about. Confirmed pass-1 issues are listed at the bottom — re-test them and tag them so I can close out the pass-1 TODO.
