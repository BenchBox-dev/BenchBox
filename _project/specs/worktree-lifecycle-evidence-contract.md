...
```json
{
  ...
  "worktree": {
    ...
    "registered": true,
    "git_state": "clean | dirty | locked | detached | missing | unregistered",
    "branch": "fix/example",
    "head_sha": "abc123..."
  },
  "integration_evidence": [
    {
      ...
      "source": "gh api /repos/{id}/pulls/1914"
    }
  ],
  "task_run_evidence": {
    "available": false,
...
| --- | --- | --- |
| `uncertain` | The default. Evidence collected is a signal, not proof of integration, or evidence collection was incomplete. | No |
| `verified-integrated` | §7's exact merged-PR evidence was found and validated for at least one structural branch, and no dirty/locked/unregistered git state contradicts it. | No — see §9; even this state is not deletion authority |
| `unavailable` | A required evidence source could not be reached or returned an error (GitHub API failure, ambiguous `git worktree list --porcelain` parse, schema_version mismatch). | No |

There is no fourth state and no default that means "safe." Any collector
error, timeout, ambiguous parse, or unrecognized `schema_version` MUST
resolve to `unavailable`, never silently to `uncertain` or
`verified-integrated`. A missing or malformed field inside a record makes
that record `unavailable` as a whole; partial trust in a corrupt record is
not permitted.
...
- **`clean`** (no uncommitted changes) — a worktree between edits, or one
...
1. **Exact PR head** — `pr_head_sha` matches a commit that was, at some
   point, the tip of the worktree's branch (from reflog, PR event history,
   or the collector's own observation; not merely "some commit reachable
   from the branch today," since force-push and rebase can change history
   after the PR closed).
...
Absence of PR evidence is `uncertain`, not `unmerged` and not
`abandoned` — the PR may exist in a paginated result the collector didn't
reach, or a merge may have happened by a path other than the GitHub PR flow
(local admin merge, mirrored push). Only a positive, exact match under
points 1–4 may set `classification.state` to `verified-integrated`; every
other outcome for that branch stays `uncertain`.
...
"report_authority": {
  ...
  "note": "A snapshot describes state observed at generated_at. It expires
    the instant new commits, pushes, PR activity, or task/run events occur.
    No automation may treat any record in this file as license to act;
    an operator reads it and, if they choose to act, re-verifies the
    specific worktree by hand immediately before acting."
}
```
...
— it is strictly a local, disposable, re-runnable snapshot. `verified-
integrated` from a report generated an hour ago is not evidence for a
decision made now; the report format itself does not distinguish "stale by
one minute" from "stale by one month," and no future phase may add an
implicit freshness/authority tier to it without revising this document.
...
already uses (`init_repo_with_origin(tmp_path)`: a bare "origin" plus a
working clone, entirely under `tmp_path`, no network, no real GitHub call).
...
| --- | --- | --- |
| `fixture_clean_active_worktree` | Registered, clean, branch present, no PR yet | `uncertain` (signal: `clean`, no integration evidence) |
| `fixture_dirty_worktree` | Registered, uncommitted changes present | `uncertain`; also exercises `worktree-remove`'s existing dirty refusal as a boundary check |
| `fixture_locked_worktree` | Registered, `git worktree lock` held | `uncertain`; exercises the locked-refusal boundary |
| `fixture_detached_worktree` | Registered, detached `HEAD` (no symbolic ref) | `uncertain`; exercises the detached-refusal boundary |
| `fixture_missing_directory` | Registered in `git worktree list` but the path no longer exists on disk | `unavailable` (collector cannot inspect git state) |
| `fixture_unregistered_path` | An arbitrary directory that is not a registered worktree at all | `unavailable`; also the exact-path-only precedent `worktree-remove` already enforces |
| `fixture_gone_upstream_squash_merged` | Branch tracking ref reports `[gone]`; a canned PR JSON fixture shows `merged: true`, matching head, base `develop`, and a `merge_commit_sha` that **is** reachable from the fixture `develop` tip | `verified-integrated` for `develop` — the one fixture that should pass §7 in full |
| `fixture_gone_upstream_no_pr_evidence` | Branch tracking ref reports `[gone]`, but no matching PR fixture exists (simulates manual remote branch deletion) | `uncertain` — proves `[gone]` alone never yields `verified-integrated` |
