...
## Update — 2026-04-27 evening: hash fix verified

After fixing D1 (the validator/submitter directory-scope hash mismatch)
in commit b03a19d76 (per-file hashing on both sides), opened PR #31
(`test/ci-validation-fixed-hash`) against `published-results` carrying
the fix + a fresh bundle generated by the patched `benchbox submit`.

Result: workflow **PASSED**. Comment:

> ## Submission Validation: PASSED
> | Bundle | Status | Benchmark | Platform | Scale |
> |--------|--------|-----------|----------|-------|
> | `tpch_sf001_duckdb_sql_20260427_195343_6c7d3792.json` | PASS | tpch | DuckDB | SF 0.01 |

The corpus-inventory check then surfaced "Missing from inventory: ..."
(expected — the test PR didn't update results-data/corpus-inventory.json).
After running `python scripts/generate_corpus_inventory.py --write` and
committing the regenerated inventory, the full workflow went green.

Capability matrix (updated):

| Capability                                     | Status   |
|------------------------------------------------|----------|
| Trigger filter (paths + base branch)           | OK       |
| Hash mismatch error rendering                  | OK       |
| Schema validation error rendering              | OK       |
| Happy-path validator PASS (post-fix)           | OK (PR #31) |
| Corpus inventory check enforces freshness      | OK       |
| PR-comment update (idempotent, same body)      | not exercised — second push touched only the manifest, which the workflow's `grep -v submission-manifest.json` filter excludes from `changed_bundles`, so the validate step was skipped and the comment wasn't touched |
| Fork PR permissions                            | not exercised |

PR #31 closed without merging. Coverage gaps that remain (idempotent
re-comment + fork PR permissions) are filed below for a follow-up
re-run.
