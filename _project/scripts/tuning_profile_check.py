...
def main(argv: list[str] | None = None) -> int:
    ...
    for platform in platforms:
        for benchmark in benchmarks:
            template_path = REPO_ROOT / "examples" / "tunings" / platform / f"{benchmark}_tuned.yaml"
            if not template_path.exists():
                print(f"{platform}/{benchmark}: missing template {template_path.relative_to(REPO_ROOT)}")
                failures += 1
                continue
            ...
            print(
                ...
                f"mechanisms={mechanisms}; hash={metadata['tuning_template_hash']}"
            )
