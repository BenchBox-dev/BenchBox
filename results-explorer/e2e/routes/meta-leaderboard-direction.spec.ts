...
test("matrix direction uses one reading rule across every visible column", async ({ page }) => {
  ...
  await expect(page.getByRole("radio", { name: "Speedup" })).toHaveAttribute("aria-checked", "true");
  ...
  for (let index = 0; index < headerCount; index += 1) {
    ...
    await expect(header).toContainText("Speedup vs best · 1.00x is best; lower is worse");
    await expect(header).toContainText("Native:");
  }
