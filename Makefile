...
skill-sync:
	...
	@# even under `-n` (recursive dry-run passthrough). The case guard below
	@# must therefore stay in the SAME logical line: its `exit 0` is what
	@# stops `make -n skill-sync` and `make -n guards-fix` from really
	@# executing the apply and mutating the skill mirrors. For a real
	@# read-only preview, run `tools/skill-sync preview` directly.
	@case " $(MAKEFLAGS) " in \
		*" n "*|*" -n "*|*" --just-print "*) echo "dry-run (-n): skipping skill-sync"; exit 0 ;; \
	esac; \
...
release-cut: .release-cut-tree-required
	...
	git rm -f --ignore-unmatch .pre-commit-config.yaml .importlinter todo.config.yaml skill-sync.conf tools/skill-sync .gitattributes .coveragerc_core .dockerignore .env.example .mcp.json AGENTS.md CLAUDE.md GEMINI.md ANTIGRAVITY.md
