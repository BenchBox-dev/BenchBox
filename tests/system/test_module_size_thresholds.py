...
ALLOWLIST = {
    ...
    ): 2_820,  # CLI tuning + query subset + validation + visualization + help + compression + interactive wizard + post-run charts + driver_version/auto_install options + --publish flag + --benchmark-option + --iterations + explicit utf-8 encoding + run() McCabe 285 → <18 extraction helpers + benchmark-aware --scale defaulting (PR #351 review follow-up) + --strict-translation fail-closed policy + liquid_clustering_auto strategy wiring
