...
def test_schema_versions_definitions() -> None:
    ...
    cols = checker.get_table_columns_for_version(11)
    ...
    assert "override_rules" in cols["results"]
...
def test_validate_database_schema_detects_missing_table() -> None:
    ...
    try:
        ...
        assert any("missing required tables for v11: cohort_metadata" in err for err in errors)
    finally:
        con.close()
...
def test_validate_database_schema_detects_missing_view() -> None:
    ...
    try:
        ...
        errors = checker.validate_database_schema(con, expected_version=11)
        assert any("missing required views for v11: result_detail_metrics" in err for err in errors)
    finally:
        con.close()
...
def test_cli_default_schema_checks(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "Schema v11" in captured.out
...
def test_cli_json_mode(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert data["current_version"] == 11
    assert "v11" in data["schema_checks"]
    assert "v10" not in data["schema_checks"]


def test_cli_specific_schema_version(capsys: pytest.CaptureFixture[str]) -> None:
    exit_code = checker.main(["--schema-only", "--schema-versions", "11"])
    assert exit_code == 0
    captured = capsys.readouterr()
    assert "Schema v11" in captured.out
    assert "Schema v10" not in captured.out
...
def test_cli_db_path_check(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    with duckdb.connect(str(db_file)) as con:
        for stmt in checker.generate_schema_ddl(11):
            con.execute(stmt)
        con.execute("INSERT INTO metadata VALUES (11)")
