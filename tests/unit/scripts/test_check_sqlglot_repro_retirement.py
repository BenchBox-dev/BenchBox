...
class TestNoUpgradePath:
    ...
    def test_base_ref_goes_through_merge_base(self, monkeypatch, capsys):
        seen: dict[str, str] = {}

        def fake_merge_base(ref="origin/develop"):
            seen["ref"] = ref
            return "base123"
    ...
    def test_crash_without_summary_fails_closed(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(checker, "run_repros", lambda: (2, "Traceback: boom\n"))
        assert checker.main([]) == 1
        assert "crashed" in capsys.readouterr().err

    def test_upgrade_with_all_fail_exits_zero(self, monkeypatch, capsys):
        monkeypatch.setattr(checker, "merge_base", lambda ref="origin/develop": "abc123")
        monkeypatch.setattr(
            checker,
            "locked_sqlglot_version",
