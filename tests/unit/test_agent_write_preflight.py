...
def _fake_uv(bin_dir: Path, record: Path) -> None:
    """A stand-in for `uv` that records the directory the repair ran from."""
...
def _linked_worktree(tmp_path: Path) -> tuple[Path, Path]:
    primary = _init_clone(tmp_path / "BenchBox")
    linked = primary.parent / "BenchBox.wt-hooks"
    subprocess.run(
        ["git", "worktree", "add", "-q", "-b", "fix/hook-health", str(linked), "HEAD"],
        cwd=primary,
        check=True,
    )
...
def _write_generated_hook(primary: Path, interpreter: str) -> None:
...
def test_preflight_repairs_hooks_whose_interpreter_no_longer_exists(tmp_path: Path) -> None:
    """A deleted worktree's venv leaves every linked worktree unable to commit."""
    ...
    # Repairing from the worktree is what created the breakage; it must repair
    # from the primary clone instead.
    assert record.read_text(encoding="utf-8").strip() == str(primary.resolve())


def test_preflight_leaves_a_healthy_hook_alone(tmp_path: Path) -> None:
    primary, linked = _linked_worktree(tmp_path)
    _write_generated_hook(primary, sys.executable)
    record = tmp_path / "uv-cwd.txt"
    _fake_uv(tmp_path / "fake-bin", record)
...
def test_preflight_installs_missing_hooks_in_a_linked_worktree(tmp_path: Path) -> None:
    """Repair is no longer reserved for ephemeral clones."""
    primary, linked = _linked_worktree(tmp_path)
    record = tmp_path / "uv-cwd.txt"
    _fake_uv(tmp_path / "fake-bin", record)
    ...
    assert result.returncode == 0, result.stdout + result.stderr
    assert "no pre-commit hook is installed" in result.stdout
    assert record.read_text(encoding="utf-8").strip() == str(primary.resolve())


def _fake_failing_uv(bin_dir: Path) -> None:
    """A stand-in for `uv` that fails to simulate pre-commit install failure."""
    bin_dir.mkdir(parents=True, exist_ok=True)
    fake = bin_dir / "uv"
...
def test_preflight_warns_when_hook_repair_fails(tmp_path: Path) -> None:
    """When hook repair fails, preflight issues a best-effort warning and succeeds."""
    primary, linked = _linked_worktree(tmp_path)
    _write_generated_hook(primary, str(tmp_path / "BenchBox.wt-deleted" / ".venv" / "bin" / "python"))
    _fake_failing_uv(tmp_path / "fake-bin")
    ...
    assert result.returncode == 0, result.stdout + result.stderr
    assert "pre-commit install failed/unavailable" in result.stderr
    assert "commit-time guards will not run here" in result.stderr
...
def test_worktree_removal_hook_isolation(tmp_path: Path) -> None:
    ...
    hook.write_text(
        ...
        f'echo "hook ran from $(pwd)" >> "{record}"\n'
        "exit 0\n",
        encoding="utf-8",
    )
    hook.chmod(0o755)
...
def test_preflight_repairs_sibling_hook_with_stale_interpreter(tmp_path: Path) -> None:
    """When pre-commit is healthy but a sibling hook has a dead interpreter, preflight repairs."""
    primary, linked = _linked_worktree(tmp_path)
    _write_generated_hook(primary, sys.executable)
    # Write a commit-msg hook with a dead interpreter
    hooks = primary / ".git" / "hooks"
    commit_msg = hooks / "commit-msg"
    commit_msg.write_text(
        f"#!/bin/sh\n# File generated by pre-commit\nINSTALL_PYTHON={tmp_path / 'nonexistent' / 'python'}\nexit 0\n",
        encoding="utf-8",
    )
