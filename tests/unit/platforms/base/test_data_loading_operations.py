...
class TestManifestFileSourceV2:
    ...
    def test_manifest_v2_respects_manifest_order_by_default(self, tmp_path):
        manifest_data = {
            "version": 2,
            "benchmark": "tpch",
            "scale_factor": 0.01,
...
class TestDataSourceResolver:
    ...
    def test_platform_name_propagated_to_manifest_source(self):
        ...
        manifest_src = resolver.providers[2]  # ManifestFileSource is third
        assert getattr(manifest_src, "_platform_name", None) == "snowflake"

    def test_table_mode_propagated(self):
        resolver = DataSourceResolver(table_mode="external")
        manifest_src = resolver.providers[2]
        assert getattr(manifest_src, "_table_mode", None) == "external"

    def test_platform_config_propagated(self):
        cfg = {"key": "value"}
        resolver = DataSourceResolver(platform_config=cfg)
        manifest_src = resolver.providers[2]
        assert getattr(manifest_src, "_platform_config", None) is cfg
