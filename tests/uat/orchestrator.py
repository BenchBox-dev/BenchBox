...
def _canonical_datagen_source(benchmark: str) -> str:
    """Return the benchmark whose datagen cache ``benchmark`` actually consumes.

    Benchmarks that reuse another's dataset (``read_primitives`` and
    ``write_primitives`` resolve to ``tpch``; ``tpcds_obt`` resolves to
    ``tpcds``) declare it as ``data_source`` in the benchmark registry, the
    same contract ``benchbox/cli/orchestrator.py`` resolves the shared datagen
    root through. Keying reuse by the requested workload name instead would
    treat one cache as several and re-reserve its full footprint per alias.

    Unknown names fall back to themselves: an unregistered benchmark reserves
    its own growth, which is the conservative direction for a disk guard.
    """
    try:
        from benchbox.core.benchmark_registry import get_benchmark_metadata

        metadata = get_benchmark_metadata(benchmark.lower())
    except Exception:  # pragma: no cover - registry import/lookup is advisory here
        return benchmark.lower()
    if not metadata:
        return benchmark.lower()
    return str(metadata.get("data_source") or benchmark).lower()


def _datagen_cache_present(source_benchmark: str, scale: float) -> bool:
    """Report whether the datagen cache for one source/scale already exists.

    Reuse is a property of the filesystem, not of this process: a rerun over
    datasets generated by an earlier sweep reuses them, and a cell that failed
    before generating anything leaves nothing to reuse. Probing the cache
    covers both, where a set of completed cells covers neither.
    """
    try:
        from benchbox.utils.path_utils import get_benchmark_runs_datagen_path

        path = get_benchmark_runs_datagen_path(source_benchmark, scale)
...
def _build_disk_floor_runner(
    ...
    *,
    ...
    datagen_cache_probe: Callable[[str, float], bool] | None = None,
) -> CellRunner:
    ...
    Datagen reuse is keyed by the canonical data-source benchmark and confirmed
    against the datagen cache on disk, so an alias workload does not re-reserve
    a footprint its source already generated, a rerun over existing datasets
    does not reserve growth it will not create, and a cell that died before
    generating anything does not mark its source as available for later cells.
    ...
    datagen_present = datagen_cache_probe or _datagen_cache_present

    def runner(platform: str, benchmark: str, scale: float, **kwargs) -> CellResult:
        prediction = None
        source_key = (_canonical_datagen_source(benchmark), scale)
        if watch_disk_floor and budget_table is not None:
            already_present = source_key in seen_datagen_sources or datagen_present(*source_key)
        ...
        if prediction is not None and datagen_present(*source_key):
            seen_datagen_sources.add(source_key)
        return result
