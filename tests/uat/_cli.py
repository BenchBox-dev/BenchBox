...
Digest provenance (uat-evidence-provenance-binding): gate-check recomputes
sha256 over the stage artifact files the orchestrator hashed at sweep time
and rejects a mismatch or an absent digest. This is integrity against
accident/drift, not tamper-proof provenance (digests are computed on the
operator machine and can be regenerated by the same hand that forges
evidence -- without signing infra). See
_project/release-evidence/README.md.

These are intentionally thin: argparse to call into framework module,
...
def _handle_gate_check(args: argparse.Namespace) -> int:
    ...
    # Digest provenance (w2): recompute sha256 of each stage artifact from the
    # stage dirs passed on the command line (not from the committed evidence
    # file itself — see _digest_for_stage_file). Mismatch or absence mismatches
    # become a HOLD reason alongside the existing SHA-equality / dirtiness /
    # floors / ordering checks (all unchanged, per must-preserve). An older
    # summary without artifact_digests has already been marked HOLD by
    # build_combined_evidence above with a regenerate message, so this
    # comparison covers only summaries that carry digests; unknown SHAs are
    # similarly already marked HOLD there, but the recomputation check is
    # defense-in-depth so a hand-edited "not-unknown" SHA still lands in a
    # digest mismatch unless the file bytes also match.
    ...
    for summary, stage_dir in zip(summaries, [p.parent for p in stage_paths]):
        if summary.artifact_digests is None:
            continue  # already a HOLD via build_combined_evidence's regenerate message
