...
(`.github/workflows/seed-corpus.yml`) runs on a monthly cron (`0 7 1 * *`,
01:00 UTC on the 1st) and remains callable via `workflow_dispatch`. Its matrix
regenerates live comparable local cells plus admitted remaining-gap cells; the
authoritative cell list is the workflow file, summarized in
`SEED_CORPUS_SPEC.md`.
