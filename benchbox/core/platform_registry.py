...
"ducklake": {"display_name": "DuckLake", "description": "Open lakehouse format • DuckDB engine • Parquet + SQL catalog", "category": "olap", "libraries": [{"name": "duckdb", "required": true}], "requirements": ["duckdb>=1.3.0"], "installation_command": "uv add duckdb", "adoption": "emerging", "supports": ["olap", "lakehouse", "columnar", "parquet"], "driver_package": "duckdb", "notes": "Catalog metadata backend is selectable (DuckDB-file default, SQLite, or self-hosted PostgreSQL) and Parquet DATA_PATH may be local or S3. Requires a live DuckDB runtime >= 1.3 for the ducklake extension.", "capabilities": {"supports_sql": true, "supports_dataframe": false, "default_mode": "sql", "platform_family": "duckdb", "inherits_from": "duckdb", "default_deployment": "local", "deployment_modes": {"local": {"mode": "local", "display_name": "DuckLake Local", "description": "Local DuckDB-file (or SQLite) catalog with Parquet on local disk", "requires_credentials": false, "requires_cloud_storage": false, "requires_network": false, "default_for_platform": true, "dependencies": ["duckdb"], "auth_methods": []}, "postgres_catalog": {"mode": "self-hosted", "display_name": "DuckLake PostgreSQL Catalog", "description": "Self-hosted PostgreSQL catalog metadata (via DuckDB's postgres extension) with Parquet DATA_PATH", "requires_credentials": true, "requires_cloud_storage": false, "requires_network": true, "default_for_platform": false, "dependencies": ["duckdb"], "auth_methods": ["password"]}, "cloud_storage": {"mode": "local", "display_name": "DuckLake Cloud Storage", "description": "Any catalog backend with S3-backed Parquet DATA_PATH via DuckDB's httpfs extension", "requires_credentials": false, "requires_cloud_storage": true, "requires_network": true, "default_for_platform": false, "dependencies": ["duckdb"], "auth_methods": ["credential_chain", "access_key"]}}}, "support_status": "experimental"},
