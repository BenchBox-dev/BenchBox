...
"motherduck": {"display_name": "MotherDuck", "description": "Serverless DuckDB cloud • Managed • Cloud storage", "category": "cloud", "libraries": [{"name": "duckdb", "required": true}], "requirements": ["duckdb>=0.9.0"], "installation_command": "uv add duckdb", "adoption": "emerging", "supports": ["olap", "cloud", "columnar", "serverless"], "driver_package": "duckdb", "capabilities": {"supports_sql": true, "supports_dataframe": false, "default_mode": "sql", "platform_family": "duckdb", "inherits_from": "duckdb", "cost_class": "paid_credits", "default_deployment": "managed", "deployment_modes": {"managed": {"mode": "managed", "display_name": "MotherDuck Cloud", "description": "Serverless DuckDB in MotherDuck cloud", "requires_credentials": true, "requires_cloud_storage": false, "requires_network": true, "default_for_platform": true, "dependencies": ["duckdb"], "auth_methods": ["token"]}}}, "required_credentials": ["MOTHERDUCK_TOKEN", "database"], "support_status": "beta"},
"ducklake": {"display_name": "DuckLake", "description": "Open lakehouse format • DuckDB engine • Parquet + SQL catalog", "category": "olap", "libraries": [{"name": "duckdb", "required": true}], "requirements": ["duckdb>=1.3.0"], "installation_command": "uv add 'benchbox[ducklake]'", "adoption": "emerging", "supports": ["olap", "lakehouse", "columnar", "parquet"], "driver_package": "duckdb", "notes": "Two independent axes, not a fixed set of deployments: catalog metadata backend (DuckDB-file default, SQLite, or self-hosted PostgreSQL) x Parquet DATA_PATH location (local or S3). Any combination is valid. Requires a live DuckDB runtime >= 1.3 for the ducklake extension; install via the ducklake extra, which pins that floor.", "capabilities": {"supports_sql": true, "supports_dataframe": false, "default_mode": "sql", "platform_family": "duckdb", "inherits_from": "duckdb", "default_deployment": "local", "deployment_modes": {"local": {"mode": "local", "display_name": "DuckLake Local", "description": "Local catalog (DuckDB-file or SQLite) + Parquet on local disk. Descriptive capability metadata: the adapter selects the catalog backend from --platform-option catalog=<duckdb|sqlite|postgres> and the storage location from the data_path scheme.", "requires_credentials": false, "requires_cloud_storage": false, "requires_network": false, "default_for_platform": true, "dependencies": ["duckdb"], "auth_methods": []}, "local_catalog_s3": {"mode": "local", "display_name": "DuckLake Local Catalog + S3 Storage", "description": "Local catalog (DuckDB-file or SQLite) + Parquet DATA_PATH on S3 via DuckDB's httpfs extension. Descriptive capability metadata: the adapter selects the catalog backend from --platform-option catalog=<duckdb|sqlite|postgres> and the storage location from the data_path scheme.", "requires_credentials": false, "requires_cloud_storage": true, "requires_network": true, "default_for_platform": false, "dependencies": ["duckdb"], "auth_methods": ["credential_chain", "access_key"]}, "postgres_catalog": {"mode": "self-hosted", "display_name": "DuckLake PostgreSQL Catalog", "description": "Self-hosted PostgreSQL catalog metadata (via DuckDB's postgres extension) + Parquet on local disk. Descriptive capability metadata: the adapter selects the catalog backend from --platform-option catalog=<duckdb|sqlite|postgres> and the storage location from the data_path scheme.", "requires_credentials": true, "requires_cloud_storage": false, "requires_network": true, "default_for_platform": false, "dependencies": ["duckdb"], "auth_methods": ["password"]}, "postgres_catalog_s3": {"mode": "self-hosted", "display_name": "DuckLake PostgreSQL Catalog + S3 Storage", "description": "Self-hosted PostgreSQL catalog metadata + Parquet DATA_PATH on S3. Previously unrepresentable: the old three-mode model forced a choice between the catalog axis and the storage axis. Descriptive capability metadata: the adapter selects the catalog backend from --platform-option catalog=<duckdb|sqlite|postgres> and the storage location from the data_path scheme.", "requires_credentials": true, "requires_cloud_storage": true, "requires_network": true, "default_for_platform": false, "dependencies": ["duckdb"], "auth_methods": ["password", "credential_chain", "access_key"]}}}, "support_status": "beta"},
